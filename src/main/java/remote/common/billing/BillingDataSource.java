...
public class BillingDataSource implements LifecycleObserver, PurchasesUpdatedListener,
     ...
     */
    private BillingDataSource(@NonNull Context context, String[] knownInappSKUs,
    ...
    private void loadStateCache() {
        BillingCache.INSTANCE.getAllSkuInfo(skuInfoList -> {
            for (BillingCache.SkuInfo info: skuInfoList) {
                if (info != null && info.getSku() != null && info.getStatus() != null) {
                    MutableLiveData<SkuState> state = skuStateMap.get(info.getSku());
                    if (state != null) {
                        state.postValue(info.getStatus());
                    }
                }
            }
            return null;
            ...
            if (null == skuStateLiveData) {
                ...
                switch (purchase.getPurchaseState()) {
                    case Purchase.PurchaseState.PENDING:
                        ...
                        BillingCache.INSTANCE.updateSkuState(purchaseSku, SkuState.SKU_STATE_PENDING);
                        break;
                    case Purchase.PurchaseState.UNSPECIFIED_STATE:
                        skuStateLiveData.postValue(SkuState.SKU_STATE_UNPURCHASED);
                        BillingCache.INSTANCE.updateSkuState(purchaseSku, SkuState.SKU_STATE_UNPURCHASED);
                        break;
                    case Purchase.PurchaseState.PURCHASED:
                        if (purchase.isAcknowledged()) {
                            skuStateLiveData.postValue(
                                    SkuState.SKU_STATE_PURCHASED_AND_ACKNOWLEDGED);
                            BillingCache.INSTANCE.updateSkuState(purchaseSku, SkuState.SKU_STATE_PURCHASED_AND_ACKNOWLEDGED);
                        } else {
                            skuStateLiveData.postValue(SkuState.SKU_STATE_PURCHASED);
                            BillingCache.INSTANCE.updateSkuState(purchaseSku, SkuState.SKU_STATE_PURCHASED);
                        }
                        break;
                    default:
    ...
    private void setSkuState(@NonNull String sku, SkuState newSkuState) {
        ...
        if (null == skuStateLiveData) {
            ...
            BillingCache.INSTANCE.updateSkuState(sku, newSkuState);
        }
    }
    ...
    public void launchBillingFlow(Activity activity, @NonNull String sku,
        ...
        if (null != skuDetails) {
            if (null != upgradeSku && upgradeSku.length() > 0) {
                billingClient.queryPurchasesAsync(BillingClient.SkuType.SUBS,
                        (br, purchasesList) -> {
